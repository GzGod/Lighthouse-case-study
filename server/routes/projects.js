...
module.exports = function(pool) {
  ...
  router.post('/', authMiddleware, async (req, res) => {
    const { name, logo, budget, impressions, cpm, er, cpe, tag, is_baseline, tweets, slug } = req.body;
    ...
    const { rows } = await pool.query(
      ...
      [name, logo||'', budget||0, impressions||0, cpm||0, er||0, cpe||0, tag||'', is_baseline??1, mx[0].m+1, tweets||0, slug||'']
    );
    res.json({ id: rows[0].id });
  });

  router.put('/:id', authMiddleware, async (req, res) => {
    const fields = ['name','logo','budget','impressions','cpm','er','cpe','tag','is_baseline','sort_order','tweets','slug'];
